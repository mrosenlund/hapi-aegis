'use strict';

const Hapi = require('@hapi/hapi');
const Lab = require('@hapi/lab');
const { expect } = require('@hapi/code');

const Aegis = require('../..');
const NoSniff = require('../../lib/middlewares/noSniff');

const { describe, it } = exports.lab = Lab.script();

describe('noSniff middleware', () => {

    it('returns { header: X-Content-Type-Options, value: nosniff }', () => {

        expect(NoSniff()).to.equal({ header: 'X-Content-Type-Options', value: 'nosniff' });
    });

    it('takes no options', () => {

        expect(NoSniff()).to.equal(NoSniff({ anything: true }));
    });

    it('sets the header on 200 responses', async () => {

        const server = Hapi.server();
        await server.register(Aegis);
        server.route({ method: 'GET', path: '/', handler: () => 'ok' });

        const res = await server.inject('/');
        expect(res.statusCode).to.equal(200);
        expect(res.headers['x-content-type-options']).to.equal('nosniff');
    });

    it('sets the header on Boom error responses', async () => {

        const server = Hapi.server();
        await server.register(Aegis);
        server.route({
            method: 'GET',
            path: '/boom',
            handler: () => {

                throw new Error('intentional');
            }
        });

        const res = await server.inject('/boom');
        expect(res.statusCode).to.equal(500);
        expect(res.headers['x-content-type-options']).to.equal('nosniff');
    });

    it('can be disabled via server-level options', async () => {

        const server = Hapi.server();
        await server.register({ plugin: Aegis, options: { noSniff: false } });
        server.route({ method: 'GET', path: '/', handler: () => 'ok' });

        const res = await server.inject('/');
        expect(res.headers['x-content-type-options']).to.not.exist();
    });

    it('can be disabled on a single route via plugins.aegis config', async () => {

        const server = Hapi.server();
        await server.register(Aegis);

        server.route({
            method: 'GET',
            path: '/off',
            options: { plugins: { aegis: { noSniff: false } } },
            handler: () => 'ok'
        });
        server.route({ method: 'GET', path: '/on', handler: () => 'ok' });

        const offRes = await server.inject('/off');
        const onRes = await server.inject('/on');

        expect(offRes.headers['x-content-type-options']).to.not.exist();
        expect(onRes.headers['x-content-type-options']).to.equal('nosniff');
    });
});
